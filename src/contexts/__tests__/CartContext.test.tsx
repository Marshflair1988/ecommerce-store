...
const TestActionsComponent = () => {
  ...
  return (
    <div>
      ...
      <button data-testid="remove-from-cart" onClick={handleRemoveFromCart}>
        Remove from Cart
      </button>
      <button data-testid="update-quantity" onClick={handleUpdateQuantity}>
        Update Quantity
      </button>
      <button data-testid="clear-cart" onClick={handleClearCart}>
        Clear Cart
      </button>
    </div>
  );
  ...
  test("calculates total with regular price when no discount", () => {
    const TestComponentNoDiscount = () => {
      ...
      return (
        <div>
          <div data-testid="cart-total">{cart.getCartTotal()}</div>
          <button data-testid="add-no-discount" onClick={handleAddToCart}>
            Add No Discount
          </button>
        </div>
      );
